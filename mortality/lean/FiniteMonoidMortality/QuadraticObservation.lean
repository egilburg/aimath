...
import FiniteMonoidMortality.ReachableSpans
...
/-!
# Short quadratic observations

Symmetric n-by-n matrices have n(n+1)/2 coordinates. Adjoining one constant coordinate makes the trace defect linear and yields a word witness of at most this length.
-/
...
/-- A generator fixes the scalar coordinate and acts by matrix congruence. -/
...
theorem exists_short_quadratic_observation_ne_zero
  ...
  have horbit : obs (linearWord T long seed) ≠ 0 := by
    simpa [T, seed, obs, linearWord_quadraticObservationLetter] using hlong
  ...
  have hzero : ∀ x ∈ boundedLinearOrbitSpan T seed
    ...
    | mem x hx =>
        ...
        simpa [T, seed, obs, linearWord_quadraticObservationLetter] using hwzero
    | zero => simp
