import SierpinskiFormal.FiniteMortalityCompression
import SierpinskiFormal.ShortQuadraticObservation
import SierpinskiFormal.FiniteMortalityDescent
import SierpinskiFormal.MatrixWordScalarExtension
...
Finiteness means that the entire set of word products is finite. The proof
uses the existing actual-word span theorem, finite compressed returns, an
invariant quadratic form, and explicit sandwich rank descent. It proves an
exponential bound and does not assert the quadratic Rystsov conjecture.
-/
...
theorem exists_short_rank_decreasing_sandwich_of_finite
  ...
  obtain ⟨Q, hQsymm, hQtrace, hQinv⟩ :=
    exists_invariant_quadraticForm_returnMatrix_of_finite
  ...
  have heval (w : List A) :
      ell (matrixWord M w * B * (matrixWord M w).transpose) =
        Matrix.trace (returnMatrix M U V w * Q *
          (returnMatrix M U V w).transpose) := by
    simp only [ell, traceSandwichMatrixDual_apply, B, returnMatrix,
  ...
  refine ⟨w, hwlen, ?_⟩
  apply rank_resetSandwich_lt_of_invariantTraceDefect_ne_zero M h w U V hfac Q (hQinv w)
