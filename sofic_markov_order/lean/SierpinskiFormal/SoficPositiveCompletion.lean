import SierpinskiFormal.SoficPairChain
import SierpinskiFormal.StationaryWordMarkov
...
def completionResidual (e : Fin D ≃ Set.powersetCard Q 2) : Module.End ℝ (Q → ℝ) :=
  uniformMean - completionEpsilon (Q := Q) D •
    ∑ s : Fin (D - 1), soficPairChain (Pi.basisFun ℝ Q) e s
...
theorem completionResidual_entry_pos (e : Fin D ≃ Set.powersetCard Q 2)
  ...
  have hsum : (∑ s : Fin (D - 1),
      endEntry (soficPairChain (Pi.basisFun ℝ Q) e s) i j) ≤ (D - 1 : ℕ) := by
    calc
  ...
  change (Fintype.card Q : ℝ)⁻¹ - completionEpsilon (Q := Q) D *
    (∑ s : Fin (D - 1), endEntry (soficPairChain (Pi.basisFun ℝ Q) e s) i j) > 0
  linarith
