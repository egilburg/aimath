import SierpinskiFormal.IIDPairFactor
import SierpinskiFormal.SoficPositiveCompletion
...
abbrev coordinateSelectorSource (I A : Type*) [Fintype I] [Fintype A] [Nonempty A] :
    FiniteStateSource I A where
  Edge _ := A
  edgeFintype _ := inferInstance
  edgeNonempty _ := inferInstance
  target i _ := i
  label _ a := a
...
def uniformSelectorLaw (T : A → Module.End ℝ (Q → ℝ))
  ...
  weight := sourceSelectorWeights (uniformEmission T)
  nonneg F := Finset.prod_nonneg (fun ij _ => uniformEmission_nonneg T h0 ij (F ij))
  sum_eq_one := by
    classical
    have h := (coordinateSelectorSource (Q × Q) A).sum_selectorWeight_eq_one
      (uniformEmission T) (uniformEmission_sum T hsum)
    have hfin : (coordinateSelectorSource (Q × Q) A).selectorFintype =
        (inferInstance : Fintype ((Q × Q) → A)) := Subsingleton.elim _ _
    rw [hfin] at h
    exact h
...
theorem uniformSelector_marginal (T : A → Module.End ℝ (Q → ℝ))
  ...
  classical
  have h := (coordinateSelectorSource (Q × Q) A).sum_selectorWeight_mul_apply
...
theorem uniformSoficMeasure_stationary (T : A → Module.End ℝ (Q → ℝ))
    (h0 : ∀ a i j, 0 ≤ endEntry (T a) i j) (hsum : ∑ a, T a = uniformMean) :
    MeasurePreserving digitShift (uniformSoficMeasure T h0 hsum)
