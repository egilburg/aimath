import SoficMarkovOrder.IIDWordLaw
import SoficMarkovOrder.ProcessMarkov
...
theorem pairProcess_shift (code : B → B → A) :
    pairProcess code ∘ sequenceShift = sequenceShift ∘ pairProcess code := rfl

theorem pairOutput_prefix (code : B → B → A) (n : ℕ) (ω : ℕ → B) :
    pairOutput code (orbitPrefix sequenceHead sequenceShift (n+1) ω) =
      orbitPrefix sequenceHead sequenceShift n (pairProcess code ω) := by
  induction n generalizing ω with
  | zero => rfl
  | succ n ih =>
      change code (ω 0) (ω 1) :: pairOutput code
        (orbitPrefix sequenceHead sequenceShift (n+1) (sequenceShift ω)) = _
      rw [ih]
      rfl

theorem mem_wordCylinder_iff_prefix (w : List A) (ω : ℕ → A) :
    ω ∈ wordCylinder w ↔ orbitPrefix sequenceHead sequenceShift w.length ω = w := by
  induction w generalizing ω with
  | nil => simp [wordCylinder, orbitPrefix]
  | cons a w ih =>
      change (ω 0 = a ∧ sequenceShift ω ∈ wordCylinder w) ↔
        ω 0 :: orbitPrefix sequenceHead sequenceShift w.length (sequenceShift ω) = a :: w
...
theorem pairFactorMeasure_stationary (L : FiniteProbabilityWeights B) (code : B → B → A) :
    MeasurePreserving sequenceShift (pairFactorMeasure L code) (pairFactorMeasure L code) := by
  refine ⟨measurable_sequenceShift, ?_⟩
  unfold pairFactorMeasure
  rw [Measure.map_map measurable_sequenceShift (measurable_pairProcess code),
    ← pairProcess_shift, ← Measure.map_map (measurable_pairProcess code) measurable_sequenceShift,
    L.measurePreserving_sequenceShift_iidMeasure.map_eq]
...
theorem pairFactorMeasure_cylinder (L : FiniteProbabilityWeights B) (code : B → B → A)
  ...
  have hset : pairProcess code ⁻¹' wordCylinder w =
      {ω | pairOutput code (orbitPrefix sequenceHead sequenceShift (w.length+1) ω) = w} := by
    ext ω
  ...
  rw [L.integral_comp_orbitPrefix_sequence (w.length+1)
