...
Both commands returned **exit status 0**. There were **192 compiled project-local modules**, matching the full packaged source closure; Publication.olean was produced in this build. Lake reports “Build completed successfully (3679 jobs)”. This counts Lake jobs, not AI workers. All 4 endpoint axiom lists contain only `propext`, `Classical.choice`, `Quot.sound`; no sorryAx or unproved project axiom supports them. Exact types and proof expressions are retained in the audit output. Deprecation/style warnings in the build logs are not suppressed or presented as proof errors.
